...
1. Verify prerequisites: `claude` CLI, `jq`, and at least one worker agent via `bash plugins/delegate-coder/skills/delegate-coder/scripts/detect.sh`. If any is missing, stop and tell me what to install.
...
5. On my go-ahead, run the full benchmark:
   `cd benchmark && REPO_DIR=<TARGET_REPO> SKILL_SRC=$(pwd)/../plugins/delegate-coder/skills/delegate-coder REPS=3 bash run_benchmark.sh`
...
- The worker agent (e.g. MiMo) must be authenticated before the run; see plugins/delegate-coder/skills/delegate-coder/references/setup.md.
