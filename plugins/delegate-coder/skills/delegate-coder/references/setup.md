...
- `contract` — local Ollama settings. All numeric limits must be strictly positive. The router requires a Git worktree, a clean target/worktree, an isolated feature/delegate branch, and a bounded objective test. Contract setup adds only `/.claude/delegate-coder.log` to `.git/info/exclude` and removes the legacy broad `/.claude/` rule if present, so the audit log remains available to `/delegate stats` without hiding tracked or other `.claude/*` changes.
