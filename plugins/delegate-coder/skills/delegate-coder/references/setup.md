...
- `command_override` — optional. Full shell command per mode; use `{task}` as the placeholder for the task spec. When set, it replaces the built-in adapter (required when `agent` is `custom`). Example: `"exec": "mytool go --prompt '{task}' --auto"`. Note that the escalation rule (stop after 2 identical failures) is enforced by the orchestrating agent per `SKILL.md` instructions, not by the scripts themselves.
