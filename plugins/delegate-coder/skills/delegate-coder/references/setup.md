...
- `test_command` — optional. The project's verification command; the orchestrator runs this after every exec delegation. `/delegate-setup` auto-detects it via `scripts/detect-test.sh` (npm/pytest/go/cargo/make) and asks you to confirm. Omit if the project has no test suite. `test_command` runs in a subshell and must be self-contained (ensure full `PATH` or use absolute binary paths, e.g. `/usr/local/bin/flutter test`).
...
Contract mode is an opt-in local Ollama backend for Claude/Codex-style heavy orchestrators. Invoke it explicitly with `bash plugins/delegate-coder/skills/delegate-coder/scripts/delegate.sh contract` or select `"implementation_backend": "contract"` for JSON `exec` contracts. It requires a local Ollama server with `qwen3-coder:30b` available, unless `DELEGATE_MODEL` selects another local model. Pass one object or a JSON array for sequential contracts. The router creates or uses an isolated feature/delegate branch before the first write, rejects a dirty worktree or target, validates strict structured output, writes transactionally, and restores failed candidates. It returns a pre-contract target-only diff, metrics, and final test log, makes at most one self-correction attempt after a failed test, reports non-PASS outcomes as `NOOP`, `FAIL`, `TEST_FAIL`, or `PREFLIGHT_FAIL` (saving the candidate output to `${TMPDIR:-/tmp}/delegate-coder-candidate.XXXXXX` for operator inspection), and rejects context-truncated responses. `context_files` entries are capped at 64KB per file and 256KB total across all context files; when a reference file exceeds the cap, inline the required names, constants, and type signatures directly in the contract instructions spec. Passing a small, working test file in `context_files` also serves as a structural template so local models maintain framework scaffolding instead of emitting bare assertions. `DELEGATE_NUM_CTX`, `DELEGATE_KEEP_ALIVE`, `DELEGATE_CURL_TIMEOUT`, and `DELEGATE_TEST_TIMEOUT` override the configured local limits.
