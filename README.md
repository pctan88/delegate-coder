...
> [!IMPORTANT]
> **Sequential Contract Workflow:** The contract router enforces strict workspace cleanliness before execution. If running sequential contracts on the same file, note that each contract should target one file per branch. You must checkout the base branch (or commit/stash changes) between contracts on the same file to ensure the worktree is clean for the subsequent run.
